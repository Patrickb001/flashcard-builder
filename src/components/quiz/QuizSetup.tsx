...
const STYLES: { id: QuestionStyle; name: string; blurb: string }[] = [
  {
    id: "recall",
    name: "Recall",
    blurb: "One fact per question, four options",
  },
  {
    id: "vignette",
    name: "PANCE style",
...
export default function QuizSetup({
  ...
  return (
    <div className="quiz quiz-setup">
      ...
      {notice && (
        <div
          className={`ai-notice ${noticeFailed ? "failed" : "partial"}`}
          role="alert"
        >
          <strong>
            {noticeFailed
              ? "No questions were written"
              : "Some cards were left out"}
          </strong>
          <span>{notice}</span>
        </div>
      )}
      ...
      {ai.mode === "off" && (
        <div className="quiz-ai-gate">
          <p className="muted">
            Questions are written for you once and then kept. That first step
            needs the AI helper switched on — after that, taking a test works
      ...
      {ai.mode !== "off" && unwritten.length > 0 && (
        <div
          className={`ai-notice ${pool.length === 0 ? "partial quiz-write-cta" : "partial"}`}
          role="status"
        >
          <strong>
            {pool.length === 0
          ...
          <div className="form-actions">
            ...
            >
              Write {unwritten.length} {styleNoun} question
              {unwritten.length === 1 ? "" : "s"}
            </button>
          </div>
        </div>
      )}
      ...
      {pool.length === 0 ? (
        ...
        <>
          ...
          <p className="muted small">
            Drawn from {pool.length} question{pool.length === 1 ? "" : "s"},
            favouring the ones you have seen least.
          </p>
        </>
      )}
